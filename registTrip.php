   ...
   <div class="area-msg"><?php err('common')?></div>
    <label class="<?php if(!empty($err_msg['title'])) echo 'err'; ?>">
     タイトル<span class="label-require">必須</span>
      <input type="text" class="regist-title" name="title" value="<?php echo getFormData('title'); ?>">
      <p class="resist-exp">例:東京タワーに登ってきました</p>
    </label>
    <div class="area-msg"><?php err('title')?></div>
     <label class="<?php if(!empty($err_msg['place_id'])) echo 'err'; ?>">
       都道府県を選択<span class="label-require">必須</span>
      <select name="place_id" class="regist-pref">
        <option value="0" <?php if(getPlace('place_id') == 0 ){ echo 'selected'; } ?> >選択する</option>
        <?php foreach($dbPlaceData as $key => $val){ ?>
         <option value="<?php echo $val['place_id'] ?>" <?php if(getFormData('place_id') == $val['place_id'] ){ echo 'selected'; } ?> >
           <?php echo $val['place']; ?>
          </option>
        <?php } ?>
      </select>
      ...
      <label class="<?php if(!empty($err_msg['season'])) echo 'err'; ?>">
        季節を選んでください<span class="label-require">必須</span>
          <group class="inline-radio">
            <div><input type="radio" id="select1" name="season" value="1"  <?php if(getFormData('season') == 1 ){ echo 'checked'; } ?>><label for="select1">春</label></div>
            <div><input type="radio" id="select2" name="season" value="2"  <?php if(getFormData('season') == 2 ){ echo 'checked'; } ?>><label for="select2">夏</label></div>
            <div><input type="radio" id="select3" name="season" value="3" <?php if(getFormData('season') == 3 ){ echo 'checked'; } ?>><label  for="select3">秋</label></div>
            <div><input type="radio" id="select4" name="season" value="4" <?php if(getFormData('season') == 4 ){ echo 'checked'; } ?>><label for="select4">冬</label></div>
          </group>
      </label>
      ...
      <label class="<?php if(!empty($err_msg['season'])) echo 'err'; ?>">
        どんな場所？（複数選択可）<span class="label-require">必須</span>
          <group class="inline-radio">
            <div><input type="checkbox" id="emotion1" name="emotion[]" value="1" <?php if(strpos(getFormData('emotion'),'1') !== false){ echo 'checked'; } ?>><label for="emotion1">絶景</label></div>
            <div><input type="checkbox" id="emotion2" name="emotion[]" value="2" <?php if(strpos(getFormData('emotion'),'2') !== false){ echo 'checked'; } ?>><label for="emotion2">楽しい</label></div>
            <div><input type="checkbox" id="emotion3" name="emotion[]" value="3" <?php if(strpos(getFormData('emotion'),'3') !== false){ echo 'checked'; } ?>><label for="emotion3">驚き</label></div>
            <div><input type="checkbox" id="emotion4" name="emotion[]" value="4" <?php if(strpos(getFormData('emotion'),'4') !== false){ echo 'checked'; } ?>><label for="emotion4">神秘的</label></div>
            <div><input type="checkbox" id="emotion5" name="emotion[]" value="5" <?php if(strpos(getFormData('emotion'),'5') !== false){ echo 'checked'; } ?>><label for="emotion5">魅力的</label></div>
          </group>
      </label>
      <div class="area-msg"><?php err('place_id') ?></div>
      <label class="<?php if(!empty($err_msg['comment'])) echo 'err'; ?>">
        内容を書いてください（空白可）
        <textarea name="comment" id="js-count" value="例：東京タワーに行ってきました" class="regist-contain" cols="50" rows="10" style="height:150px;"><?php echo getFormData('comment'); ?></textarea>
      </label>
    ...
    <input type="text" name="maptitle" class="maptitle" value="<?php echo getFormData('maptitle'); ?>" size="20" id="place">
    <button type="button" onclick="search()" class="mapserch-btn">検索</button><br />
    <div class="mapdit">
      緯度：<input type="text" id="ido" name="ido" value="<?php echo getFormData('ido'); ?>" readonly>
      経度：<input type="text" id="keido" name="keido" value="<?php echo getFormData('keido'); ?>" readonly>
    </div>
      ...
      <input type="submit" id="js-delete" class="btn <?php if(isset($class)) echo $class; ?>" value="削除する" name="tripdelete">
    </form>
   </div>
  </section>
<!--mapapi処理読み込み-->
